...
if(isset($_POST['GoVisitorPage'])){
 ...
 <link href="assets/styles/bootstrap.css" rel="stylesheet">
   ...
   <a class="navbar-brand col-md-3 col-lg-2 mr-0 px-3" href="#">STG</a>
     ...
     <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
         ...
         <div class="row mt-4">
           ...
           if($userdata['level_p'] > 0){
             $result3 = mysqli_query($link, "SELECT *,INET_NTOA(user_ip) AS user_ip FROM users_test WHERE user_id = $visitorId LIMIT 1");
             $userdata2 = mysqli_fetch_array($result3);
             ...
             $result=mysqli_query($link, "SELECT user_id, f_name, l_name, user_login,  case when level_p = 1 then 'Сотрудник' when level_p = 2 then 'Менеджер' when level_p = 3 then 'Администратор' end level FROM users_test where user_id = $visitorId");
             ...
             $userOneQuery=mysqli_query($link, "SELECT user_id, task_id, executor_id, task_theme, task_text FROM Tasks, users_test WHERE Tasks.executor_id = $visitorId and Tasks.executor_id = users_test.user_id and Tasks.status = 'false'");
             ?><?
             echo "
             </div>
             <div>
         ...
         echo "
           ...
           <div class='row'>";
           $userList = $userdata['user_id'];
           $userOneQuery=mysqli_query($link, "SELECT user_id, task_id, executor_id, task_theme, task_text FROM Tasks, users_test WHERE Tasks.executor_id = $visitorId and Tasks.executor_id = users_test.user_id and Tasks.status = 'true'");
